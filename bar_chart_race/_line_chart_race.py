...
class _LineChartRace(CommonChart):
    def __init__(
        ...
        self.agg_line_kwargs, self.agg_line_label = self.get_line_kwargs(
            agg_line_kwargs, "agg"
        )
        self.others_line_func = others_line_func
        self.others_line_kwargs, self.others_line_label = self.get_line_kwargs(
            others_line_kwargs, "others"
        )
        ...
        self.scale = scale
        self.writer = self.get_writer(metadata=None, fps=None)
        ...
        self.image_radius = self.fig.get_figwidth() * self.fig.dpi * 0.02
    ...
    def get_line_kwargs(self, kwargs, kind):
        ...
        if kwargs is None:
            ...
            raise TypeError(
                f"{kind}_line_kwargs must be a dictionary with line properties"
            )
    ...
    def prepare_data(self, df):
        ...
        orientation = "h"
        values, ranks = prepare_wide_data(
            df,
            orientation,
            sort,
            self.n_lines,
    ...
    def get_colors(self, colors):
        ...
        if isinstance(colors, str):
            ...
            raise TypeError(
                "`colors` must be a string name of a colormap, a matplotlib colormap "
    ...
    def get_images(self, images):
        ...
        if len(images) != len(self.df_values.columns):
            raise ValueError(
                "The number of images does not match the number of columns"
            )
    ...
    def get_visible(self, i):
        ...
        return (self.df_ranks.iloc[i] <= self.n_lines + n + 0.5).to_dict()
    ...
    def anim_func(self, i):
        ...
        for col, collection in self.collections.items():
            ...
            color_arr[:, -1] = np.clip(
                color_arr[:, -1] * self.fade, self.min_fade, None
            )
    ...
    def init_func(self):
        ...
        if self.agg_line is not None:
            ...
            lc = LineCollection(
                [[(x, val)]], colors=[color], linewidths=[lw], linestyles=[ls]
            )
            collection = ax.add_collection(lc)
        ...
        if self.others_agg_line is not None:
            ...
            collection = ax.add_collection(lc)
            text = ax.text(
                x + x_extra, val, label, ha="left", va="center", size="smaller"
            )
        ...
        if self.images:
            for col in self.df_values.columns:
                ...
                circle = mpatches.Circle(
                    center, transform=None, radius=self.image_radius, fill=None, lw=0
                )
                ax.add_patch(circle)
                left, right = xpixel - self.image_radius, xpixel + self.image_radius
                bottom, top = ypixel - self.image_radius, ypixel + self.image_radius
    ...
    def make_animation(self):
        ...
        anim = FuncAnimation(
            self.fig, self.anim_func, frames, self.init_func, interval=interval
        )
