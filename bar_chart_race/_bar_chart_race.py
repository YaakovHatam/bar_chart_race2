...
class _BarChartRace(CommonChart):
    ...
    def validate_params(self):
        ...
        if self.bar_textposition not in ("outside", "inside", None):
            raise ValueError(
                '`bar_textposition` must be one of "outside", "inside" or None'
            )
    ...
    def get_bar_kwargs(self, bar_kwargs):
        ...
        if "width" in bar_kwargs or "height" in bar_kwargs:
            raise ValueError(
                "Do not set the width or height with `bar_kwargs`. "
    ...
    def get_period_label(self, period_label):
        ...
        if self.orientation == "h":
            default_period_label["x"] = 0.95
            default_period_label["y"] = 0.15 if self.sort == "desc" else 0.85
            default_period_label["ha"] = "right"
            default_period_label["va"] = "center"
        else:
            default_period_label["x"] = 0.95 if self.sort == "desc" else 0.05
            default_period_label["y"] = 0.85
    ...
    def get_col_filt(self):
        ...
        if self.n_bars < self.df_ranks.shape[1]:
            ...
            if orient_sort in [("h", "asc"), ("v", "desc")]:
                # 1 is high
                col_filt = (self.df_ranks < self.n_bars + 0.99).any()
            else:
                # 1 is low
        ...
        if not self.filter_column_colors:
            if not self.col_filt.all():
                ...
                if (col_idx_ct > exp_ct).any():
                    warnings.warn(
                        "Some of your columns never make an appearance in the animation. "
    ...
    def get_subplots_adjust(self):
        ...
        if self.bar_textposition == "outside":
            ...
            if self.fixed_max:
                ...
                if self.orientation == "h":
                    self.fixed_max_value = ax.transData.inverted().transform(
                        (end_pixel, 0)
                    )[0]
                else:
                    self.fixed_max_value = ax.transData.inverted().transform(
    ...
    def plot_bars(self, ax, i):
        ...
        if self.orientation == "h":
            ...
            if not self.fixed_max and self.bar_textposition == "outside":
                ...
                new_max_pixels = (
                    ax.transData.transform((max_bar, 0))[0] + self.extra_pixels
                )
            ...
            if not self.fixed_max and self.bar_textposition == "outside":
                max_bar = bar_length.max()
                new_max_pixels = (
                    ax.transData.transform((0, max_bar))[1] + self.extra_pixels
                )
    ...
    def add_bar_labels(self, ax, bar_location, bar_length):
        if self.bar_textposition:
            ...
            delta = 0.01 if self.bar_textposition == "outside" else -0.01
            ...
            for x1, y1 in zipped:
                ...
                text_obj = ax.text(
                    xtext, ytext, text, clip_on=True, **self.bar_label_font
                )
    ...
    def add_perpendicular_bar(self, ax, bar_length, i):
        if self.perpendicular_bar_func:
            ...
            if not ax.lines:
                if self.orientation == "h":
                    ax.axvline(val, lw=10, color=".5", zorder=0.5)
                else:
                    ax.axhline(val, lw=10, color=".5", zorder=0.5)
            else:
                line = ax.lines[0]
                if self.orientation == "h":
